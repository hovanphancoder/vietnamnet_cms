...
class PostsController extends ApiController
{
    ...
    // list
    public function lists($posttype_slug = '', $page = 1) {
        try {
            $page = (int) $page;
            if(empty($page)) {
                $this->error_v2('Page valid');
            }
            if(empty($posttype_slug)) {
                $this->error_v2('Posttype not empty');
            }
            
            $posttype = $this->_check_posttype($posttype_slug);
            if(empty($posttype)) {
                $this->error_v2('Posttype not found');
            }
            ...
            $default_fields = ['id', 'title', 'slug', 'created_at', 'updated_at'];
            $fields_name_allow = array_merge($fields_name_allow, $default_fields);
            ...
            $this->success_v2(['data' => $data, 'is_next' => $is_next, 'page' => $page, 'limit' => $this->limit], 'Posts list');
        } catch (AppException $e) { 
            $this->error_v2($e->getMessage(), [], 500);
        } catch (\Exception $e) {
            $this->error_v2('Internal server error', [], 500);
        }
    }
    // detail
    public function detail($posttype_slug = '', $id = '') {
        try {
            ...
            if(empty($post)) {
                $this->error_v2('Post not found');
            }
            $post = $this->_formatPosts($posttype_slug, [$post], true);
            
            $post = $post[0];
            $this->success_v2($post, 'Post detail');
        } catch (AppException $e) {
            $this->error_v2($e->getMessage(), [], 500);
        } catch (\Exception $e) {
            $this->error_v2('Internal server error', [], 500);
    ...
    public function update_view($posttype_slug = '', $id = '') {
        try {
            ...
            if($update_status) {
                $this->success_v2(['message' => 'Update view success'], 'Update view success');
            } else {
                $this->error_v2('Update view failed', [], 500);
            }
            } catch (AppException $e) {
                $this->error_v2($e->getMessage(), [], 500);
            } catch (\Exception $e) {
