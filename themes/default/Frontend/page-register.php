...
Render::asset('css', '/themes/cmsfullform/Frontend/Assets/css/register_styles.css', ['area' => 'frontend', 'location' => 'head']);
...
$meta
    ->title(option('site_register_title', APP_LANG))
    ->description(option('site_register_description', APP_LANG))
    ->robots('index, follow')
...
$meta
    ...
    ->custom('<meta name="author" content="' . option('site_register_title', APP_LANG) . '">')
...
$meta
    ->og('locale', $locale)
    ->og('type', 'website')
    ->og('title', option('site_register_title', APP_LANG))
    ->og('description', option('site_register_description', APP_LANG))
    ->og('url', base_url('register'))
    ->og('site_name', option('site_register_title', APP_LANG))
    ->og('updated_time', date('c'));

// Add Twitter Card tags
$meta
    ->twitter('card', 'summary_large_image')
    ->twitter('title', option('site_register_title', APP_LANG))
    ->twitter('description', option('site_register_description', APP_LANG))
    ->twitter('site', '@' . option('site_register_title', APP_LANG));
...
$schemaData = [
    'site' => [
        'title' => option('site_register_title', APP_LANG),
        'desc' => option('site_register_description', APP_LANG),
...
$registerSchema = new WebPage([
    ...
    'potentialAction' => [
        ...
        'target' => [
            '@type'       => 'EntryPoint',
            'urlTemplate' => base_url('/account/register/')
        ]
    ],
...
get_header([
    'meta' => $meta->render(),
    'schema' => $registerSchema->render()

]);
?>

<?php get_template('sections/auth/registerform2'); ?>
