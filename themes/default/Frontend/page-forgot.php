...
Render::asset('css', '/themes/cmsfullform/Frontend/Assets/css/forgotpass_styles.css', ['area' => 'frontend', 'location' => 'head']);
...
$meta
    ->title(option('site_forgot_pass_title', APP_LANG))
    ->description(option('site_forgot_pass_description', APP_LANG))
    ->robots('index, follow')
    ->canonical(base_url('forgot-pass'));

// Add basic meta tags
$meta
    ->custom('<meta name="generator" content="CMSFullForm">')
    ->custom('<meta name="language" content="' . APP_LANG . '">')
    ->custom('<meta name="author" content="' . option('site_forgot_pass_title', APP_LANG) . '">')
...
$meta
    ->og('locale', $locale)
    ->og('type', 'website')
    ->og('title', option('site_forgot_pass_title', APP_LANG))
    ->og('description', option('site_forgot_pass_description', APP_LANG))
    ->og('url', base_url('forgot-pass'))
    ->og('site_name', option('site_forgot_pass_title', APP_LANG))
    ->og('updated_time', date('c'));

// Add Twitter Card tags
$meta
    ->twitter('card', 'summary_large_image')
    ->twitter('title', option('site_forgot_pass_title', APP_LANG))
    ->twitter('description', option('site_forgot_pass_description', APP_LANG))
    ->twitter('site', '@' . option('site_forgot_pass_title', APP_LANG));
...
$schemaData = [
    'site' => [
        'title' => option('site_forgot_pass_title', APP_LANG),
        'desc' => option('site_forgot_pass_description', APP_LANG),
...
$forgotSchema = new WebPage([
    'name'        => option('site_forgot_pass_title', APP_LANG),
    'description' => option('site_forgot_pass_description', APP_LANG),
    ...
    'breadcrumb'  => [
        ...
        'itemListElement' => [
            ...
            [
                ...
                'position' => 2,
                'item'     => [
                    '@id'  => base_url('/account/forgot-password/'),
                    'name' => option('site_register_title', APP_LANG)
                ]
            ]
        ]
    ],
    'potentialAction' => [
        '@type'  => 'ResetPassAction ',
        'target' => [
            '@type'       => 'EntryPoint',
            'urlTemplate' => base_url('/account/forgot-password')
        ]
    ],
...
<?php get_template('sections/auth/forgotform2'); ?>
