...
<div id="global-loader">
  <svg class="w-12 h-12 text-primary" style="animation:spin 1s linear infinite"
  ...
  <div
    ...
    <div class="sticky top-0 z-50 flex flex-wrap justify-between items-center bg-card rounded-xl p-4 mb-4 border shadow-sm" id="files-header">
      ...
      <div class="w-full md:w-6/12 mt-2 md:mt-0">
        <div class="flex justify-start space-x-2">
          <!-- Select All Checkbox -->
          <div class="mx-1 mt-2 md:mt-0 flex items-center">
            <input
              type="checkbox"
              id="selectAll"
              class="h-4 w-4 text-primary focus:ring-primary border-input rounded"
              @change="toggleSelectAll()" />
            <label for="selectAll" class="ml-2 text-sm text-muted-foreground cursor-pointer">
              <?= Flang::_e('Select All') ?>
            </label>
          </div>
          <!-- Upload Files -->
          <div class="mx-1 mt-2 md:mt-0">
            <button @click="clickMenuUpload" class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2">
          ...
          <div class="mx-1 mt-2 md:mt-0" v-if="selectedItems.length > 0">
            <button @click="deleteSelectedItems" class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-destructive text-destructive-foreground hover:bg-destructive/90 h-10 px-4 py-2">
          ...
          <div class="mx-1 mt-2 md:mt-0"
            ...
            <button @click="returnSelectedFile" class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-green-600 text-white hover:bg-green-700 h-10 px-4 py-2">
      ...
      <div class="w-full md:w-6/12 mt-2 md:mt-0">
        <div class="flex justify-end space-x-2">
          <div class="relative w-full h-10">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" data-lucide="search" class="lucide lucide-search absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-muted-foreground">
          ...
          <button
            ...
            :class="viewType === 'list' ? 'bg-primary text-primary-foreground' : 'border border-input bg-background hover:bg-accent hover:text-accent-foreground text-foreground'"
            class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 h-10 px-3 py-2">
            <i class="bi bi-list h-4 w-4"></i>
          </button>
          <button
            @click="toggleView('grid')"
            :class="viewType === 'grid' ? 'bg-primary text-primary-foreground' : 'border border-input bg-background hover:bg-accent hover:text-accent-foreground text-foreground'"
            class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 h-10 px-3 py-2">
    ...
    <div class="bg-card rounded-xl p-4 relative border shadow-sm">
      ...
      <div
        ...
        class="absolute selection-rectangle z-40 rounded"></div>
      ...
      <div v-if="viewType === 'grid'" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
        <div
          ...
          :class="[
              'item border p-4 rounded-lg text-center cursor-pointer relative transition-colors hover:bg-accent',
              isSelected(item) ? 'bg-accent border-primary' : 'bg-card border-border'
            ]">
          <!-- Checkbox -->
          <div class="absolute top-2 left-2 flex items-center">
            <input
              ...
              class="h-4 w-4 text-primary focus:ring-primary border-input rounded" />
            <!-- Số thứ tự hiển thị bên cạnh checkbox -->
            <span v-if="isSelected(item) && multi" class="ml-1 bg-primary text-primary-foreground rounded-md px-1 py-0.5 text-xs font-medium min-w-[16px] text-center">
          ...
          <div v-else>
            <i class="bi bi-file-earmark-fill text-muted-foreground text-5xl"></i>
          </div>
          <div class="truncate mt-2 text-sm font-medium text-foreground" style="max-width: 100%;">{{ item.name }}</div>
          <div v-if="item.size" class="text-xs text-muted-foreground">{{ (item.size / 1024).toFixed(2) }} KB</div>
          <div class="text-xs text-muted-foreground" :title="'Created at: ' + item.created_at">{{ item.created_at }}</div>
          <div class="text-xs text-muted-foreground" :title="'Updated at: ' + item.updated_at">{{ item.updated_at }}</div>
        </div>
      </div>
            ...
            <table class="w-full caption-bottom text-sm">
              <thead class="[&_tr]:border-b">
                <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                  <th class="px-4 py-3 text-center align-middle bg-muted text-muted-foreground font-medium w-12">Select</th>
                  <th class="px-4 py-3 text-left align-middle bg-muted text-muted-foreground font-medium">Name</th>
                  <th class="px-4 py-3 text-left align-middle bg-muted text-muted-foreground font-medium">Size</th>
                  <th class="px-4 py-3 text-left align-middle bg-muted text-muted-foreground font-medium">Created</th>
                  <th class="px-4 py-3 text-left align-middle bg-muted text-muted-foreground font-medium">Updated</th>
                </tr>
              </thead>
              <tbody class="[&_tr:last-child]:border-0">
                <tr
                  ...
                  @contextmenu.prevent.stop="showContextMenu(item, $event)"
                  :class="[
                      'item border-b transition-colors hover:bg-muted/50 cursor-pointer',
                  ...
                  <td class="px-4 py-3 align-middle text-center">
                    <div class="flex items-center justify-center">
                      <input
                        ...
                        class="h-4 w-4 text-primary focus:ring-primary border-input rounded" />
                      <!-- Số thứ tự hiển thị bên cạnh checkbox -->
                      <span v-if="isSelected(item) && multi" class="ml-2 bg-primary text-primary-foreground rounded-md px-1 py-0.5 text-xs font-medium min-w-[16px] text-center">
                        {{ selectedOrder[item.id] }}
                      </span>
                    </div>
                  </td>
                  ...
                  <td class="px-4 py-3 align-middle">
                    <div class="flex items-center space-x-3">
                      ...
                      <div v-else>
                        <i class="bi bi-file-earmark-fill text-muted-foreground text-3xl"></i>
                      </div>
                      <span class="text-foreground font-medium">{{ item.name }}</span>
                    </div>
                  </td>
                  <td class="px-4 py-3 align-middle text-muted-foreground">{{ item.size ? (item.size / 1024).toFixed(2) + ' KB' : '-' }}</td>
                  <td class="px-4 py-3 align-middle text-muted-foreground">{{ item.created_at }}</td>
                  <td class="px-4 py-3 align-middle text-muted-foreground">{{ item.updated_at }}</td>
                </tr>
              </tbody>
            </table>
          </div>
    ...
    <div class="flex justify-between items-center bg-card rounded-xl p-4 mt-4 border shadow-sm">
      ...
      <button
        @click="clickPrevPage"
        :disabled="page_now === 1"
        class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2 disabled:bg-muted disabled:text-muted-foreground disabled:cursor-not-allowed">
        <?= Flang::_e('prev') ?>
      </button>
    ...
    <div
      ...
      class="context-menu absolute shadow-lg rounded-lg py-2 w-48 z-50 border border-border !bg-white">
      <ul>
        <li v-if="checkImages(selectedItem?.name)" class="px-4 py-2 cursor-pointer hover:bg-accent text-popover-foreground transition-colors" @click="clickMenuCopyLink">
          <i class="bi bi-link-45deg h-4 w-4 mr-2"></i> <?= Flang::_e('copy image link') ?>
        </li>
...
<script>
  ...
  const initTheme = () => {
    // Lấy theme từ parent window nếu có
    try {
      const parentTheme = window.parent.document.documentElement.getAttribute('data-theme') ||
        window.parent.document.documentElement.classList.contains('dark') ? 'dark' : 'light';
      updateTheme(parentTheme);
    } catch (e) {
      // Fallback nếu không thể truy cập parent
      updateTheme('light');
    }
  };
  ...
  createApp({
    ...
    setup() {
      ...
      window.addEventListener('message', function(event) {
        ...
        // Xử lý theme changes từ parent window
      ...
      const handleClickOutside = (event) => {
        ...
        if (!isClickInsideMenu && contextMenuVisible.value) {
          // Use setTimeout to ensure the click event on menu items is processed first
          setTimeout(() => {
      ...
      // Watch for changes in selectedItems to update Select All state
      watch([selectedItems], () => {
      ...
      const clickMenuCopyLink = async () => {
        if (selectedItem.value) {
          const imageUrl = `${BASE_URL}uploads/${selectedItem.value.path}`;
          ...
          try {
            ...
            if (show_log) {
              console.log('Image link copied to clipboard:', imageUrl);
            }

            // Optional: Show a toast notification
            // You can replace this with your preferred notification system
            alert('<?= Flang::_e('image link copied to clipboard') ?>');
          } catch (err) {
      ...
      const toggleSelectAll = () => {
        ...
        const allItems = sortedData.value;
      ...
      const updateSelectAllState = () => {
        ...
        const allItems = sortedData.value;
        const allChecked = allItems.length > 0 && allItems.every(item => selectedItems.value.includes(item.id));
        const someChecked = allItems.some(item => selectedItems.value.includes(item.id));
      ...
      const selectItemsInRectangle = () => {
        ...
        updateSelectAllState(); // Cập nhật trạng thái Select All
      };
