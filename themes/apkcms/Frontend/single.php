...
System\Libraries\Render::asset('css', 'css/single.css', ['area' => 'frontend', 'location' => 'head']);
        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex flex-col lg:flex-row gap-0 lg:gap-8 pt-0 lg:pt-6">
                    ...
                    <div class="mb-6 w-full lg:w-[760px]">
                        ...
                        <nav class=" mb-4 lg:flex flex-col sm:flex-row sm:items-center sm:justify-between text-sm  gap-2 sm:gap-0" aria-label="Breadcrumb">
                            ...
                            <div class="flex flex-wrap items-center">
                                ...
                                <a href="/" class="flex lg:hidden items-center text-sky-600 mr-2">
                                        ...
                                        <?php foreach ($post_categories as $index => $category): ?>
                                            <?php if (isset($category['name']) && isset($category['slug'])): ?>
                                                <a href="<?= link_cat($category['slug'], $post['posttype'] ?? 'posts') ?>" 
                                                   class="text-sky-600 text-[16px] font-bold transition-colors  text-sm sm:text-base hover:text-[#0a569d]">
                            ...
                            <div class="text-gray-500 hidden lg:block text-xs sm:text-sm">
                    ...
                    <div class="prose prose-lg max-w-none">
                        ...
                        <div class="text-gray-p space-y-4 font-arial text-base leading-relaxed">
                            ...
                            if (!empty($recommend_posts) && is_array($recommend_posts)) {
                                $recommend_posts = $recommend_posts['data'];
                                //add thêm class để ul li hiện dấu chấm phía trước
                                echo '<ul class="recommend-posts list-disc">';
                                foreach ($recommend_posts as $recommend_post) {
                                    echo '<li class="leading-[1.5] ml-6">';
                                    echo '<h3 class="">';
                                    echo '<a href="'.link_single($recommend_post['slug'] ?? '', $recommend_post['posttype'] ?? $post['posttype']).'" class="text-sky-700 font-bold hover:text-sky-600  transition-colors">';
                                    echo $recommend_post['title'] ?? '';
                                    echo '</a>';
                                    echo '</h3>';
                                    echo '</li>';
                                }
                            ...
                            <?php if (!empty($posts_data) && is_array($posts_data)): ?>
                                <ul class="space-y-3 ml-0">
                                    ...
                                    <li class="flex items-start">
                                        <span class="text-sky-600 font-bold mr-3 mt-1 text-[8px] leading-[14px]">■</span>
                ...
                <div class="w-full lg:w-1/3 space-y-6  pt-10 w-[300px]">
                    ...
                    <div class="lg:sticky lg:top-6 space-y-6">
                        ...
                        <div class="pt-4">
                            <h2 class="text-lg notosans-bold text-sky-600 uppercase font-bold border-b border-[#add2ff] mb-4 pb-2">TRENDING POSTS</h2>
                            
                            <?php
                            // Lấy related posts cho sidebar
                            $sidebar_related_posts = [];
                            if (!empty($post['id']) && !empty($post['posttype'])) {
                                $sidebar_related_posts = get_posts([
                                    'posttype' => $post['posttype'],
                                    'perPage' => 5,
                                    'sort' => ['views_week', 'DESC'],
                                ]);
                                // Debug: uncomment để xem dữ liệu
                                // var_dump($sidebar_related_posts);
                            }
                            ...
                            <div class="text-center my-6">
                                <a href="/" class="inline-flex items-center px-4 py-2 text-sky-600 border border-[#2d67ad] text-xs uppercase font-bold rounded-full hover:bg-[#2d67ad] hover:text-white transition-colors" title="GO BACK TO THE HOME PAGE">
            ...
            <div class="flex flex-col lg:flex-row gap-6 lg:gap-8">
                ...
                <div class="flex-1 lg:w-2/3 ">
                    ...
                    <div class="">
                        <h2 class="text-[18px] notosans-bold  text-sky-600 font-bold uppercase pt-6 border-b border-blue-200">Recommended</h2>
                        <!-- Main content ở đây -->
                        <div class="mt-6">
                            <?php
                                    ...
                                    <div class="w-[130px] sm:w-[240px] h-[86px] sm:h-40 flex-shrink-0">
                                        <a href="<?= link_single($news_post['slug'], $news_post['posttype'] ?? $post['posttype']) ?>"
                                           title="<?= htmlspecialchars($news_post['title'] ?? '') ?>">
                                            <?= _img($image_url, htmlspecialchars($news_post['title'] ?? ''), true, 'w-[130px] sm:w-[240px] h-[86px] sm:h-40 object-cover') ?>
                                        </a>
                                    </div>
